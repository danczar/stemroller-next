...
<div class="relative grow-0 shrink-0 overflow-hidden flex flex-row bg-slate-950 text-slate-300 border-solid border-t border-slate-800/50">
  <BottomBarButton Icon={CogIcon} text="Preferences" highlighted={showPreferencesPopup} onClick={() => (showPreferencesPopup = true)} />

  <div class="w-full grow-1 shrink-1 border-solid border-l border-r border-slate-800/50"></div>

  <BottomBarButton Icon={CodeIcon} text="Open Source" onClick={() => window.openSource()} />
  <BottomBarButton Icon={HeartIcon} text="Support Original Developer" flashing={showDonatePopup} onClick={() => window.openDonate()} />
</div>
